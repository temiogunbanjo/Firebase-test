...
const showResults = (results, category) => {
    ...
    six49: {
      ...
      getBallColor(number) {
        ...
        for (let i = 0; i < a.length; i++) {
          ...
          const colorIndex =
            overShoot >= 1 ? i - this.colors.length * overShoot : i;
  ...
  content.setAttribute(
    "style",
    "font-size: 28px; font-weight: 700; padding: 10px; display: flex; flex-direction: row; justify-content: center; align-items:center; flex-wrap: wrap; max-width: 100%"
  );
      ...
      <div class="d-flex cols ticket-body">
        <div class="ticket-body-row">
          ...
          <p style="margin-top: 1.5em">
            ...
            <div class="ticket-slip-container">${ticket.BetSlips.map((slip) => {
              const slipWinStatusClass = (() => slip.status)();
          ...
          ${
            !!ticket.paymentConfigs
              ? `
              <details style="margin-top: 0.6em; font-size: 14px">
                <summary style="color: dodgerblue">Possible Wins</summary>
                ${ticket.paymentConfigs
                  .map((cut, index) => {
                    return `<p class="d-flex cols" style="background-color: dodgerblue; color: white; padding: 10px;border-radius: 5px;">
                    <span>Cut ${index + 1}: N ${parseFloat(
                      cut?.potentialWinAmount
                    ).toFixed(2)}</span>
                    <span>Condition: ${cut?.sn} wins</span>
                  </p>`;
                  })
                  .join("")}
              </details>
        ...
        <div class="ticket-body-row">
          <p class="d-flex rows align-items-center" style="justify-content:space-between">
            ...
            <span class="ticket-value">${ticket.totalWonAmount || "–"}</span>
          </p>
          ${
...
function viewTicketsHandler(options = { page: 1, limit: 50 }) {
  ...
  const apiUrl = `${
    globals[globals.environment].apiBaseUrl
  }/ticket/fetch-tickets/${globals.user?.userId}?page=${options.page}&limit=${
    options.limit
  }`;
