...
function viewResultsHandler(options = { page: 1, limit: 50 }) {
  ...
  }/sport/fetch-result-history?page=${options.page}&limit=${
    options.limit
  }`;
              ...
              <div class="d-flex cols flex-grow">
                <div class="d-flex rows space-between" style="align-items: flex-start;margin-bottom: 5px;">
                  <div class="d-flex cols">
                    <h5 class="" style="margin: 0; text-transform: uppercase">${result.drawName}</h5>
                    <span style="font-size: 10px; font-weight: 600">Event: ${result?.Event?.title} (${result?.category})</span>
                  </div>
                ...
                <div class="d-flex rows space-between">
                  <span class="">${result?.homeScore} - ${result?.awayScore}</span>
                  <h5 class="" style="margin: 0;margin-left: 20px;margin-right: 4px;letter-spacing: 2px"> Week ${
                    result?.Event?.week
                  }</h5>
                </div>
              </div>
